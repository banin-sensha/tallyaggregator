...
public class PayablesController {
    ...
    private static final Logger logger = LoggerFactory.getLogger(PayablesController.class);
    ...
    @RequestMapping(value = {"/payables/{companyName}"}, method = RequestMethod.GET, produces = "application/json")
    public ResponseEntity<?> getPayables(@PathVariable String companyName) {
        ...
        try {
            result = customRepository.search("t_account_payables", -1, queryParams);
        } catch (Exception e) {
            logger.error("Exception while fetching payables data = {}", e);
            genericResponse.setStatus(StatusResponse.getErrorMessage(HttpStatus.BAD_REQUEST, "Exception while fetching payables data"));
    ...
    @RequestMapping(value={"/account/payables"}, method = RequestMethod.POST, produces = "application/json")
    public void postExpenses(@RequestBody String request) throws Exception {

        List<Map<String, Object>> accountPayables = xmlToJavaObject.accountReceiveOrPay(request, customRepository.getComanyId("Tally on Wheels"));
        Collection record = customRepository.insertCollection(accountPayables, "t_account_payables");

        logger.info("Successfully inserted account payables = {}", record);
    }
}
